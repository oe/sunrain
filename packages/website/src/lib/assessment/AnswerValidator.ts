...
export class AnswerValidator {
    ...
    try {
      ...
      for (const rule of this.rules) {
        try {
          ...
          assessmentLogger.error('VALIDATION', `Rule ${rule.name} failed`, ruleError as Error);
          errors.push({
      ...
      for (const rule of this.customRules) {
        try {
          ...
          assessmentLogger.error('VALIDATION', `Custom rule ${rule.name} failed`, ruleError as Error);
          errors.push({
      ...
      assessmentLogger.error('VALIDATION', 'Validation process failed', error as Error);
   ...
   * 批量验证答案
   */
  validateAnswers(answers: Map<string, any>, questions: Question[]): Map<string, ValidationResult> {
   ...
   * 获取验证建议
   */
  private getSuggestion(errorCode: string, question: Question): string | undefined {
    switch (errorCode) {
      case 'FIELD_REQUIRED':
        return question.type === 'multiple_choice' ? '请至少选择一个选项' : '请填写此字段';
      case 'TEXT_TOO_SHORT':
  ...
  getRulesForQuestionType(questionType: string): ValidationRule[] {
    return this.rules.filter(rule =>
      rule.name === 'required' || rule.name === questionType
    );
  }
  ...
  getHealthStatus(): {
    ...
    const status = issues.length === 0 ? 'healthy' :
                   issues.some(issue => issue.includes('missing')) ? 'error' : 'warning';
